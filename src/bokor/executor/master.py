...
@date 2014
@brief process master base class
"""
...
class Master(Configurable):
    ...
    def _execute_and_block_transmit(self, action):
        if self.bokor.token in action.ressources or action.ressources == []:
...
# PEER management
    ...
    @aPreProcess
    def pre_faulty(self, action):
        return self._execute_and_block_transmit(action)

    @aPreProcess
    def pre_ready(self, action):
        return self._execute_and_block_transmit(action)

    @aPreProcess
    def pre_unknown(self, action):
        return self._execute_and_block_transmit(action)

    @aPreProcess
    def pre_slash(self, action):
        return self._execute_and_block_transmit(action)

    @aPreProcess
    def pre_list_peers(self, action):
        return self._execute_and_block_transmit(action)
    ...
    def list_peers(self, status):
        ...
        return [{'token': p.token,
                 ...
                 'category': p.category,
                 'status': p.status}
                for p in self.bokor.transmission.peers if p.status == status]

    @aFeature
    def faulty(self):
        return self.list_peers('faulty')

    @aFeature
    def ready(self):
        return self.list_peers('ready')

    @aFeature
    def unknown(self):
        return self.list_peers('unknown')

    @aFeature
    def slash(self):
        return self.faulty() + self.unknown() + self.ready()
